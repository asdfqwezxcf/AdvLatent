...
class SquareAttack():
    ...
    def perturb(self,x,xadv,delta,c,h,w,s):
        for i in range (x.shape[0]):
            ...
            x_window = x[i,:,anchor_h:anchor_h+s,anchor_w:anchor_w+s]
            xadv_window = xadv[i,:,anchor_h:anchor_h+s,anchor_w:anchor_w+s]
            xadv_window_temp = x_window+delta[i,:,anchor_h:anchor_h+s,anchor_w:anchor_w+s]
            if self.clip_min is not None and self.clip_max is not None:
                xadv_window_temp = torch.clamp(xadv_window_temp,self.clip_min,self.clip_max)
            # prevent trying out a delta if it doesn't change x_curr (e.g. an overlapping patch)
            while torch.sum(torch.abs(xadv_window_temp-xadv_window)<1e-7) == c*s*s:
                rand_select = torch.randint(0,2,(c,1,1)).to(delta.device)
                delta[i,:,anchor_h:anchor_h+s,anchor_w:anchor_w+s] = rand_select*self.epsilon + (rand_select-1)*self.epsilon
                xadv_window_temp = x_window+delta[i,:,anchor_h:anchor_h+s,anchor_w:anchor_w+s]
                if self.clip_min is not None and self.clip_max is not None:
                    xadv_window_temp = torch.clamp(xadv_window_temp,self.clip_min,self.clip_max)
