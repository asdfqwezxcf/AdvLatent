...
class HSJA():
    ...
    def boundary_search(self,model,x,xadv,y,theta):
        ...
        if self.norm == 2:
            high = torch.ones(x.shape[0]).double().to(x.device) # cast to float64 to avoid overflow in some devices (cuda)
            threshold = torch.tensor(theta).repeat_interleave(x.shape[0]).to(x.device)
        else:
            high = distance.double() # cast to float64 to avoid overflow in some devices (cuda)
...
class SignOPT():
    ...
    def bin_search_local(self,model,x,y,theta,init_bound,threshold):
        ...
        while (high.double() - low.double()) > threshold:
            mid = (high.double() + low.double()) / 2 # ensure the precision in some device to avoid overflow and endless loop
            xadv = x + theta*mid.float()
            if self.is_adv(model,xadv,y):
                high = mid
            else:
                low = mid
...
class TriAttack():
    ...
    """
    def __init__(self,epsilon=0.1,clip_min=None,clip_max=None,target=None,iterations=200,
