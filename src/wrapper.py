...
class CodecFeatureCompressionClassifier(AnalyzableModule):
    ...
    def __init__(self, classification_model, device, encoder_config=None, codec_params=None, decoder_config=None,
        ...
        self.fc = redesign_model(classification_model, classifier_config, model_label='classification')
        self.post_transform = build_transform(post_transform_params)
    ...
    def forward(self, x):
        ...
        x = torch.stack(tmp_list).to(self.device)
        x = self.decoder(x)
        x = torch.flatten(x, 1)
        return self.fc(x)
...
def get_wrapped_classification_model(wrapper_model_config, device=None, distributed=False):
    """
    ...
    model = models.__dict__[classification_model_config['name']](**classification_model_config['params'])
    sync_bn = classification_model_config.get('sync_bn', False)
    if distributed and sync_bn:
        model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
    wrapped_model = WRAPPER_CLASS_DICT[wrapper_model_name](model, compression_model=None, device=device,
