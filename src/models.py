...
class SplitVggHead(nn.Module):
    def __init__(self,vgg,mean=(0.485,0.456,0.406),std=(0.229, 0.224, 0.225),ratio=8) -> None:
        super().__init__()
        self.model = vgg.features[:ratio]
        self.mean = torch.tensor(mean).view(3,1,1)
        self.std = torch.tensor(std).view(3,1,1)
    def forward(self,x):
...
class SplitVggTail(nn.Module):
    def __init__(self,vgg,ratio=8) -> None:
        super().__init__()
        self.features = vgg.features[ratio:]
...
class SplitResNetHead(nn.Module):
    def __init__(self,resnet,mean=(0.485,0.456,0.406),std=(0.229, 0.224, 0.225),depth=2) -> None:
        super().__init__()
        model = resnet
        sequential = OrderedDict()
        tail_dict = ['dqt','layer3','layer4','avgpool','flatten','fc']
        if depth == 3:
            tail_dict = ['dqt','layer4','avgpool','flatten','fc']
        if depth == 4:
            tail_dict = ['dqt','avgpool','flatten','fc']
        if depth == 1:
            tail_dict = ['dqt','layer2','layer3','layer4','avgpool','flatten','fc']
        if depth == 0:
            tail_dict = ['dqt','layer1','layer2','layer3','layer4','avgpool','flatten','fc']
...
class SplitResNetTail(nn.Module):
    def __init__(self,resnet,depth=2) -> None:
        super().__init__()
        model = resnet
        sequential = OrderedDict()
        tail_dict = ['dqt','layer3','layer4','avgpool','fc']
        if depth == 3:
            tail_dict = ['dqt','layer4','avgpool','fc']
        if depth == 4:
            tail_dict = ['dqt','avgpool','fc']
        if depth == 1:
            tail_dict = ['dqt','layer2','layer3','layer4','avgpool','fc']
        if depth == 0:
            tail_dict = ['dqt','layer1','layer2','layer3','layer4','avgpool','fc']
        for child_name, child_module in model.named_children():
